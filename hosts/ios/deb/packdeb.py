#!/usr/bin/env python3
"""Build operit-ios .deb (Mac has no dpkg-deb, so construct the ar package in Python).

Preserves setuid bits from the source tree (any file whose mode has 0o4000).
Computed from files/ + DEBIAN/.
"""
import os, io, tarfile, gzip, stat

ROOT = os.path.dirname(os.path.abspath(__file__))
FILES = os.path.join(ROOT, "files")
DEBIAN = os.path.join(ROOT, "DEBIAN")
OUT = os.path.join(ROOT, "operit2-ios_0.3.9_iphoneos-arm64.deb")


def tar_add(tar, path, arcname, mode=None):
    st = os.stat(path)
    ti = tarfile.TarInfo(arcname)
    ti.size = st.st_size
    ti.mtime = int(st.st_mtime)
    ti.uid = 0
    ti.gid = 0
    ti.type = tarfile.REGTYPE
    # preserve full permission bits; explicit mode wins, else source mode as-is
    ti.mode = mode if mode is not None else (st.st_mode & 0o7777)
    with open(path, "rb") as f:
        tar.addfile(ti, f)


def _is_junk(name):
    return name == ".DS_Store" or name.startswith("._")


def make_data_tar():
    buf = io.BytesIO()
    with gzip.GzipFile(fileobj=buf, mode="wb") as gz:
        with tarfile.open(fileobj=gz, mode="w") as tar:
            for dp, dns, fns in os.walk(FILES):
                dns[:] = [d for d in dns if not _is_junk(d)]
                darc = os.path.relpath(dp, FILES)
                if darc != ".":
                    st = os.stat(dp)
                    ti = tarfile.TarInfo(darc if darc.endswith("/") else darc + "/")
                    ti.type = tarfile.DIRTYPE
                    ti.mode = (st.st_mode & 0o7777) or 0o755
                    ti.uid = 0
                    ti.gid = 0
                    ti.mtime = int(st.st_mtime)
                    tar.addfile(ti)
                for fn in sorted(fns):
                    if _is_junk(fn):
                        continue
                    fp = os.path.join(dp, fn)
                    arc = os.path.relpath(fp, FILES)
                    # preserve setuid bit if present in source
                    src_mode = os.stat(fp).st_mode
                    mode = (src_mode & 0o7777) if (src_mode & 0o4000) else None
                    tar_add(tar, fp, arc, mode=mode)
    return buf.getvalue()


def compute_installed_size_kb():
    total = 0
    for dp, dns, fns in os.walk(FILES):
        dns[:] = [d for d in dns if not _is_junk(d)]
        for fn in fns:
            if _is_junk(fn):
                continue
            try:
                total += os.path.getsize(os.path.join(dp, fn))
            except OSError:
                pass
    return max(1, (total + 1023) // 1024)


def make_control_tar():
    items = []
    ctrl = open(os.path.join(DEBIAN, "control"), "rb").read()
    kb = compute_installed_size_kb()
    lines = [l for l in ctrl.split(b"\n") if not l.lower().startswith(b"installed-size:")]
    body = b"\n".join(lines).rstrip() + b"\n"
    ctrl = body + ("Installed-Size: %d\n" % kb).encode()
    items.append(("control", ctrl, 0o644))
    post = open(os.path.join(DEBIAN, "postinst"), "rb").read()
    items.append(("postinst", post, 0o755))
    prerm_path = os.path.join(DEBIAN, "prerm")
    prerm = open(prerm_path, "rb").read() if os.path.exists(prerm_path) else b"#!/bin/bash\nexit 0\n"
    items.append(("prerm", prerm, 0o755))
    buf = io.BytesIO()
    with gzip.GzipFile(fileobj=buf, mode="wb") as gz:
        with tarfile.open(fileobj=gz, mode="w") as tar:
            for name, content, mode in items:
                ti = tarfile.TarInfo(name)
                ti.size = len(content)
                ti.mode = mode
                ti.uid = 0
                ti.gid = 0
                tar.addfile(ti, io.BytesIO(content))
    return buf.getvalue()


def ar_pack(out, members):
    with open(out, "wb") as f:
        f.write(b"!<arch>\n")
        for name, data in members:
            name_field = name if len(name) <= 15 else name[:15] + "/"
            header = f"{name_field:<16}{0:<12}{0:<6}{0:<6}{'100644':<8}{len(data):<10}`\n"
            f.write(header.encode())
            f.write(data)
            if len(data) % 2:
                f.write(b"\n")


def main():
    data = make_data_tar()
    ctrl = make_control_tar()
    ar_pack(OUT, [("debian-binary", b"2.0\n"), ("control.tar.gz", ctrl), ("data.tar.gz", data)])
    print(f"wrote {OUT}  ({os.path.getsize(OUT)} bytes)")


if __name__ == "__main__":
    main()
