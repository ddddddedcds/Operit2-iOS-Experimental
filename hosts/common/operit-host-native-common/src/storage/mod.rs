...
/// Some callers pass absolute paths even though the host contract expects
/// `runtime/...` or `workspaces/...`. Accepting those paths avoids breaking
/// downstream code while still rejecting paths that escape the roots.
