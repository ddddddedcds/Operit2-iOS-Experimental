...
void main() {
  ...
  test('getSavePath passes the suggested name and accepted type groups',
      () async {
    when(
      mockApi.getSavePath(
        'some/path/',
        ...
        argThat(
          isA<FileTypes>().having(
            ...
            (FileTypes types) => types.extensions,
            'extensions',
            <String>['opsnapshot', 'zip'],
          ),
        ),
      ),
    ).thenAnswer(
        (_) => Future<String?>.value('file://saved/snapshot.opsnapshot'));

    final path = await plugin.getSavePath(
      ...
      initialDirectory: 'some/path/',
      suggestedName: 'snapshot.opsnapshot',
    );

    expect(path, 'file://saved/snapshot.opsnapshot');
  });
