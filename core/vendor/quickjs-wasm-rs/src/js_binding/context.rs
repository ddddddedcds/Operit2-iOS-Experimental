...
pub(super) static CLASSES: Lazy<Mutex<HashMap<(usize, TypeId), JSClassID>>> =
...
impl JSContextRef {
    ...
    /// Gets the class ID registered for one Rust closure type in this QuickJS runtime.
    fn get_class_id<T: 'static>(&self) -> JSClassID {
        ...
        //
        // ```
        // unsafe extern "C" fn finalize<T: 'static>(runtime: *mut JSRuntime, value: JSValue) {
        //     let pointer = JS_GetOpaque(
        //         value,
        //         *CLASSES.lock().unwrap().get(&(runtime as usize, TypeId::of::<T>())).unwrap(),
        //     ) as *mut RefCell<T>;
        ...
        *CLASSES
            .lock()
            .unwrap()
            .entry((runtime as usize, TypeId::of::<T>()))
    ...
    F: FnMut(*mut JSContext, JSValue, c_int, *mut JSValue, c_int) -> JSValue + 'static,
{
    ...
    where
        F: FnMut(*mut JSContext, JSValue, c_int, *mut JSValue, c_int) -> JSValue + 'static,
    {
        (get_rust_value::<F>(ctx, *data).unwrap().borrow_mut())(ctx, this, argc, argv, magic)
    }

    Some(trampoline::<F>)
}

/// Gets a Rust callback value using the class registration of its QuickJS runtime.
fn get_rust_value<T: 'static>(ctx: *mut JSContext, raw: JSValue) -> Result<&'static RefCell<T>> {
    unsafe {
        ...
        let pointer = JS_GetOpaque(
            ...
            *CLASSES
                .lock()
                .unwrap()
                .get(&(runtime as usize, TypeId::of::<T>()))
                .ok_or_else(|| anyhow!("QuickJS callback class is not registered"))?,
        ) as *const RefCell<T>;
