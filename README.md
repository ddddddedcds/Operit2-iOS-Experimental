...
- **第三方插件**：市场插件生态是 **Android-first** 的，iOS 上大量插件**装得上但跑不动**——安卓路径会被自动改写，但安卓专属 API（`pm` / `getprop` / `settings` / `screencap` / `svc` / `input` 等）在 iOS 上根本不存在。装前可用内置「转换分析」预判，详见下方专节与 [HANDOVER.md §16](./HANDOVER.md)。
...
市场插件（`.toolpkg`）绝大多数按 Android 编写。本 fork 提供**两层路径兼容**与**一个只读分析器**，但它们的能力边界必须清楚：

| 机制 | 做什么 | 能力边界 |
|---|---|---|
| **路径兼容层**（运行时） | 把 `/sdcard`、`/storage/emulated/0` 改写到 VFS 挂载形式；shell 命令再映射到物理目录 | 只做**字符串替换** |
| **路径转换器**（安装期） | 安装时改写 ZIP 内 `.js` 条目的路径字面量并封存 | 与上一层是**同一个函数的两个时机**，非两套机制；加密条目跳过 |
| **转换分析**（只读） | 装前报告：含多少安卓 API token、多少路径字面量、是否依赖安卓框架、结论分档 | **不转换任何东西**，只做预判 |
